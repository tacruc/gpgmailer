...
class GpgMessageConvertService {
	...
	private function signMessage(Array $sign_fingerprints, IMessage $message) {
		...
		if(empty($sign_fingerprints)) {
			return;
		}
		$sign_fingerprint = array_shift($sign_fingerprints);
		$keydataRaw = $this->gpg->export($sign_fingerprint);
		...
		$swiftmessage->getHeaders()->addParameterizedHeader('Autocrypt', '' ,['addr' => $from, 'prefer-encrypt' => 'mutual', 'keydata' => $keydata] );
